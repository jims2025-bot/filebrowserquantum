...
func StartHttp(ctx context.Context, storage *storage.Storage, shutdownComplete chan struct{}) {
	...
	api.HandleFunc("POST /service-share", withUser(serviceSharePostHandler))
	...
	api.HandleFunc("GET /heatmap/overlays", withUser(HandleGetOverlays))
	api.HandleFunc("PUT /heatmap/overlay/metadata", withUser(HandleUpdateOverlayMetadata))
