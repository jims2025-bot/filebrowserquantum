...
export async function login(username, password, recaptcha, otp) {
  ...
  if (res.status === 200) {
    ...
    // Clear backend persistent pin to ensure fresh start
    try {
      const me = await get('self');
      await update({ ...me, pinnedLocation: null }, ['pinnedLocation']);
    } catch (e) {
      console.warn("Failed to clear backend pinned location", e);
    }
